	...
	void LateUpdate()
	{
		...
		for(int i = 0; i < numMarks && i < maxMarks; i++)
			if(skidmarks[i].lastIndex != -1 && skidmarks[i].lastIndex > numMarks - maxMarks)
			{
				...
				colors[segmentCount * 4 + 0]=new Color(0, 0, 0, 1);
				colors[segmentCount * 4 + 1]=new Color(0, 0, 0, 1);
				colors[segmentCount * 4 + 2]=new Color(0, 0, 0, 1);
				colors[segmentCount * 4 + 3]=new Color(0, 0, 0, 1);
